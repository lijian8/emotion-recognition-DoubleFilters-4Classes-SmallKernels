...
def inference(images):
  ...
  # conv0 layer takes 48x48 images as input and outputs 24x24 feature maps.
  with tf.variable_scope('conv0') as scope:
    # [width, height, depth, filters]
    kernel = _variable_with_weight_decay('weights', shape=[5, 5, 1, 64], stddev=1e-4, wd=0.0)
  ...
  # pool0
  pool0 = tf.nn.avg_pool(conv0, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool0')

  # norm0
  norm0 = tf.nn.lrn(pool0, 4, bias=1.0, alpha=0.001 / 9.0, beta=0.75, name='norm0')

  # conv1 layer takes 24x24 images as input and outputs 24x24 feature maps.
  ...
  # pool1
  pool3 = tf.nn.avg_pool(norm3, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='pool3')
  ...
  with tf.variable_scope('local3') as scope:
    ...
    for d in pool3.get_shape()[1:].as_list():
      dim *= d
    reshape = tf.reshape(pool3, [FLAGS.batch_input_size, dim])
